...
#######################
Anomaly_Percent = int(sys.argv[3])
...
while boolean == False:
    ...
    if Anomaly_Percent == 0:
        ...
        RAND_CASE1 = random.randint(0, 32767) % Anomaly_Percent
...
Message_case2 = (LOOP * (RAND_CASE2) / 100)
Message_case3 = (LOOP * (RAND_CASE3) / 100)
...
with open("idRegistry.txt","r") as idfile:
    idfile1= idfile.readlines()
    # print(idfile1)
    # print(len(idfile1))
    if len(idfile1) > 0:
        for lines in idfile:
            # print("--------------------")
            currentPlace=lines[:-1]
            IDREGISTRY.add(int(currentPlace))    

with open("tcapTracking.json","r") as trackfile:
    tcapTracking=json.load(trackfile)
   
# TCAP
TCAP_VALUE = ["BEG","CONT","END"]
...
DPC_VALUE = ["2-3-4", "3-111-1", "5-5-5","2-1-1", "7-15-4", "4-19-3", "5-6-7", "3-3-3"]
...
# VALID OPCODE mapped with CATEGORY, ATYPE, ASUBTYPE
OPCODE_VALUE = {
...
# INVALID OPCODE mapped with CATEGORY, ATYPE, ASUBTYPE
...
for var in range(1,LOOP+1):
    ...
    if ((var > (Message_case1 + Message_case2)) and (var <= (Message_case1 + Message_case2 + Message_case3))):
        RAND_CDADDR = random.randint(0, 32767) % len(CDADDR_INVALID_VALUE)
        RAND_CGADDR = random.randint(0, 32767) % len(CDADDR_INVALID_VALUE)
    else:
    ...
    if (var > (Message_case1 + Message_case2 + Message_case3)):  # completely normal data
    ...
    if ((var > Message_case1) and (var <= (Message_case1 + Message_case2))):
        RAND_OPCODE = random.choice(list(INVALID_OPCODE_VALUE.keys()))
    else:
        RAND_OPCODE = random.choice(list(OPCODE_VALUE.keys()))

    OTID_VALUE=""
    DTID_VALUE=""
    OPC_VALUE_=OPC_VALUE[RAND_OPC]
    DPC_VALUE_=DPC_VALUE[RAND_DPC]

    if (TCAP_VALUE[RAND_TCAP] == "CONT" or TCAP_VALUE[RAND_TCAP] == "END") and len(IDREGISTRY)==0:
        RAND_TCAP= 0

    if TCAP_VALUE[RAND_TCAP] == "BEG":
        OTID_VALUE= random.randint(0, 32767)
        while OTID_VALUE in IDREGISTRY:
            OTID_VALUE = random.randint(0, 32767)
        key = OPC_VALUE_ +"@"+  DPC_VALUE_+"@"+ str(OTID_VALUE)
        value={"tcap": [TCAP_VALUE[RAND_TCAP]],"ids" : [OTID_VALUE]}
        tcapTracking[key] = value
        IDREGISTRY.add(OTID_VALUE)

    elif TCAP_VALUE[RAND_TCAP] == "CONT":
        OTID_VALUE=random.sample(IDREGISTRY,1)
        OTID_VALUE=OTID_VALUE[0]
        # print(IDREGISTRY)
        # print(CONTIDS)
        ...
        for keys in tcapTracking:
            if str(OTID_VALUE) in str(keys).split("@")[2]:
                # print(str(keys).split("@"))
                OPC_VALUE_=str(keys).split("@")[0]
                DPC_VALUE_=str(keys).split("@")[1]
                tcap=tcapTracking[keys]["tcap"]
                # print(IDREGISTRY)
                # print(tcapTracking)
                # print(tcap)
                if "CONT" in tcap:
                    tcap.append(TCAP_VALUE[RAND_TCAP])
                    # print(tcap)
                    tcapids=tcapTracking[keys]["ids"]
                    # print(tcapids)
                    tcapid=tcapids[-1]
                    # print(tcapid)
                    ids=list(reversed(tcapid))
                    OTID_VALUE=ids[0]
                    DTID_VALUE=ids[1]
                    tcapids.append(ids)
                    ...
                    tcap.append(TCAP_VALUE[RAND_TCAP])
                    DTID_VALUE=random.randint(0,32767)
                    while DTID_VALUE in IDREGISTRY:
                        DTID_VALUE=random.randint(0,32767)
                    IDREGISTRY.add(DTID_VALUE)
                    CONTIDS.add(DTID_VALUE)
                    idCont=[OTID_VALUE,DTID_VALUE]
                    tcapids=tcapTracking[keys]["ids"]
                    tcapids.append(idCont)
        ...
        while OTID_VALUE in CONTIDS:
            OTID_VALUE= random.sample(IDREGISTRY,1)[0]
        print(OTID_VALUE)
        for keys in list(tcapTracking):
           if str(OTID_VALUE) in str(keys).split("@")[2]:
               OPC_VALUE_=str(keys).split("@")[0]
               DPC_VALUE_=str(keys).split("@")[1]
               tcapTracking.pop(keys)
               print(type(IDREGISTRY))
               list(IDREGISTRY).remove(OTID_VALUE)
               print("Done")
               break
    ...
    if ((var > (Message_case1 + Message_case2 + Message_case3)) and (var <= LOOP)):
        MSG = {
            ...
            "OTID": OTID_VALUE,
            "DTID": DTID_VALUE,
            "TCAP": TCAP_VALUE[RAND_TCAP],
            "OPCODE": RAND_OPCODE,
            "CDTT": RAND_CDTT,
            "CDNP": RAND_CDNP,
            "CDNAI": RAND_CDNAI,
            "CDADDR": CDADDR_VALUE[RAND_CDADDR],
            "CDCN": RAND_CDCN,
            "CDLOC": CDCN_VALUE[RAND_CDCN],
            "CGTT": RAND_CGTT,
            "CGNP": RAND_CGNP,
            "CGNAI": RAND_CGNAI,
            "CGADDR": CGADDR_VALUE[RAND_CGADDR],
            "CGCN": RAND_CGCN,
            "CgLOC": CDCN_VALUE[RAND_CGCN],
            "ATYPE": OPCODE_VALUE[RAND_OPCODE][1],
            "ASUBTYPE": OPCODE_VALUE[RAND_OPCODE][2],
            "LSET": LINK_VALUE[RAND_LS],
            "CLSF": "unknown",
            "CAT": OPCODE_VALUE[RAND_OPCODE][0],
        ...
        MSG = {
            ...
            "IMSI": IMSI_VALUE[RAND_IMSI],
            "CLLI": "tklc1111101",
            "SameCountryCode": "1"  # same country  true
        }
        with open(FILENAME, "a") as tmpfile:
            tmpfile.write(str(MSG)+"\n")
        ...
        MSG = {
            ...
            "CDNP": RAND_CDNP,
            "CDNAI": RAND_CDNAI,
            "CDADDR": CDADDR_INVALID_VALUE[RAND_CDADDR],  # CDADDR INVALID
            "CDCN": RAND_CDCN,
            "CDLOC": CDCN_VALUE[RAND_CDCN],
            "CGTT": RAND_CGTT,
            "CGNP": RAND_CGNP,
            "CGNAI": RAND_CGNAI,
            "CGADDR": CGADDR_INVALID_VALUE[RAND_CGADDR],  # CGADDR INVALID
            "CGCN": RAND_CGCN,
            "CgLOC": CDCN_VALUE[RAND_CGCN],
            "ATYPE": OPCODE_VALUE[RAND_OPCODE][1],
            "ASUBTYPE": OPCODE_VALUE[RAND_OPCODE][2],
       ...
       # CASE 2 HITS
        MSG = {
            ...
            "DTID": DTID_VALUE,
            "TCAP": TCAP_VALUE[RAND_TCAP],
            "OPCODE": RAND_OPCODE,  # OPCODE invalid
            "CDTT": RAND_CDTT,
            "CDNP": RAND_CDNP,
            "CDNAI": RAND_CDNAI,
            "CDADDR": CDADDR_VALUE[RAND_CDADDR],
            ...
            "CGCN": RAND_CGCN,
            "CgLOC": CDCN_VALUE[RAND_CGCN],
            "ATYPE": INVALID_OPCODE_VALUE[RAND_OPCODE][1],
            "ASUBTYPE": INVALID_OPCODE_VALUE[RAND_OPCODE][2],
            "LSET": LINK_VALUE[RAND_LS],
            "CLSF": "unknown",
            "CAT": INVALID_OPCODE_VALUE[RAND_OPCODE][0],
            "MSISDN": MSISDN_VALUE[RAND_MSISDN],
            "IMSI": IMSI_VALUE[RAND_IMSI],
            "CLLI": "tklc1111101",
            "SameCountryCode": "0"
...
with open("idRegistry.txt","w") as tmpfile:
    for items in IDREGISTRY:
        tmpfile.write('%s\n' % items)

with open("tcapTracking.json","w") as trackfile:
    json_data=json.dumps(tcapTracking,indent=4)
    trackfile.write(json_data)
